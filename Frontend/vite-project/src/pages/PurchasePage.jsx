  ...
  const handleCreate = async (e) => {
    ...
    try {
      await api.post(PURCHASE_API, {
        ...
        totalAmount: form.totalAmount,
        purchaseDate: form.purchaseDate,
        godowns: [{ godownId: form.godownId, allocatedQuantity: parseFloat(form.quantity) }],
        invoiceUrl: form.invoiceUrl,
        note: form.note,
      });
      fetchData();
              ...
              >
                ...
                {godowns.map((g) => (
                  <option key={g._id} value={g._id}>
                    {g.location} (Available: {g.availableSpace})
                  </option>
                ))}
              </select>
              ...
              <table className="min-w-full border border-gray-200 rounded-lg">
                <thead className="bg-indigo-50 sticky top-0">
                  <tr>
                    ...
                    <th className="py-3 px-4 text-left">Godown</th>
                ...
                <tbody>
                  {purchases.map((p, idx) => (
                    ...
                    >
                      <td className="py-3 px-4">{p.supplierId?.name || "—"}</td>
                      <td className="py-3 px-4">{p.productId?.name || "—"}</td>
                      <td className="py-3 px-4 text-center">{p.quantity}</td>
                      <td className="py-3 px-4 text-center">{p.unitPrice}</td>
                      <td className="py-3 px-4 text-center">{p.totalAmount}</td>
                      <td className="py-3 px-4">{p.godowns?.[0]?.godownId?.location || "—"}</td>
                      <td className="py-3 px-4">{p.purchaseDate?.split("T")[0]}</td>
